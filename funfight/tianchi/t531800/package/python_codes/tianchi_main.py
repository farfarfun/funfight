...
def run_workflow():
    ...
    with af.config(python_job_config_1):
        job_1_cluster_serving_channel = af.cluster_serving(model_info=train_model_meta, parallelism=16)
    ...
    with af.config(global_job_config_1):
        ...
        job_2_build_index_channel = af.transform([job_2_predict_model],
                                                 executor=FlinkPythonExecutor(
                                                     python_object=BuildIndexExecutor(index_path, FloatDataType(), 2)))
